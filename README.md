This project was a test to learn how to scrape tweets from the twitter stream. I wanted to see if any correlation could be found between thermostat brands and keywords relating to where you can get rebates for the thermostats. I chose the keywords that ended up grabbing many vague and non-related tweets. This 45 minute sample from the "firehose" of the twitter stream gave me enough results that running transform commands took enough time to notice (10-30 seconds on my T420 thinkpad) but did not produce enough tweets containing multiple keywords. It was useful in many ways as practice, but I will have to look into more refined search queries and larger set of data.
...
Now with only tweets that have a keyword in the status' text, I had a suspicion that if the tweets don't have multiple keywords in each status, I woulndn't be able to find much correlation between them. To see how many keywords are in each tweet I used this code:
	...
	counter = collections.Counter(keyword_counts)
	print counter 

Which results in

	python read.py 
	Counter({1: 7370, 2: 33, 3: 2})

This tells me that trying to dig deeper won't really result in any useful data. It was useful learning how to gather the tweet data but it was too short of a timespan to find enough tweets with multiple keywords.
